...
.directive('jhAlertToast', function(AlertService, $rootScope, $translate) {
  return {
    ...
    template: '<div class="alerts" ng-cloak="">' +
      '<alert ng-cloak="" ng-repeat="alert in alerts" type="{{alert.type}}" close="alert.close()">' +
      '<h4><i class="icon fa" ng-class="alert.icon"></i> <span ng-bind="alert.title"></span></h4>' +
      '{{ alert.msg }}</alert>' +
      '</div>',
    controller: ['$scope',
    function($scope) {
      ...
      var cleanHttpErrorListener = $rootScope.$on('shikenApp.httpError', function (event, httpResponse) {
        var i;
